...
def get_args():
    parser = argparse.ArgumentParser(
        description="Train BPNet models with Cerberus using any BigWig and BED file"
    )
    ...
    parser.add_argument(
        ...
        dest="use_1024",
        action="store_true",
        help="Use BPNet1024 (2112bp -> 1024bp, tuned kernels and filters)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2114,
        help="Input sequence length (ignored when --1024 is set)",
    )
    parser.add_argument(
        "--output-len",
        type=int,
    ...
    parser.add_argument(
        ...
        "--count-pseudocount and computes the scaled pseudocount as "
        "reads × (read_length/bin_size) × (CPM factor if applicable) × target_scale.",
    )
    parser.add_argument(
        "--read-length",
        type=int,
        ...
        "ChIP-seq reads are ~150 bp, ATAC-seq fragments ~100 bp.",
    )
    parser.add_argument(
        "--input-scale",
        type=str,
        ...
        "Use 'cpm' for CPM-normalized bigWigs (requires --total-reads).",
    )
    parser.add_argument(
        "--total-reads",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Weight decay (default 0: BPNet has no normalization layers)",
    )
    parser.add_argument(
