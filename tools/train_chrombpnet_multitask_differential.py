#!/usr/bin/env python
"""Phase-2 differential fine-tuning for an existing multitask ChromBPNet.

This is the ChromBPNet analogue of ``train_multitask_differential_bpnet.py``:

1. Start from an already-trained ``MultitaskChromBPNet`` phase-1 model.
2. Reload the accessibility branch as trainable.
3. Reload the shared bias branch as frozen.
4. Fine-tune with ``DifferentialCountLoss`` on
   ``log_counts[:, cond_b] - log_counts[:, cond_a]``.

The resulting checkpoint keeps the same two absolute output heads, but its
count representation is explicitly optimized for the requested contrast.  For
regulatory interpretation, run ``tools/export_tfmodisco_inputs.py`` with
``--target-mode delta_log_counts`` on the phase-2 output directory.
"""
...
from cerberus.config import ModelConfig, PretrainedConfig, SamplerConfig, TrainConfig
...
from cerberus.pseudocount import resolve_quantile_pseudocount
...
def _select_phase2_strategy(precision_kwargs: dict) -> dict:
    """Differential count loss leaves profile-head parameters unused under DDP."""
...
def get_args() -> argparse.Namespace:
    ...
    data.add_argument(
        ...
        default=None,
        help=(
            "Optional BED/narrowPeak for phase-2 sampling. Defaults to the "
    ...
    train.add_argument(
        ...
        type=float,
        default=None,
        help="Explicit pseudocount in scaled target units; bypasses quantile estimate.",
    )
    train.add_argument(
    ...
    hw.add_argument(
        ...
        default="1",
        help=(
            "Devices passed to Lightning. Default is single-GPU because the "
            "phase-2 pseudocount is sampled before Trainer setup; use an "
            "explicit --phase2-pseudocount-override before multi-GPU DDP."
        ),
    )
...
def main() -> None:
    ...
    data_config = cfg.data_config.model_copy(
        update={
            "max_jitter": 0,
            # Keep RC augmentation consistent with phase 1; only random jitter is
            # disabled so the per-region differential target is stable.
            "reverse_complement": cfg.data_config.reverse_complement,
        }
    )
