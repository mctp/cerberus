...
def get_args():
    parser = argparse.ArgumentParser(
        description="Train Gopher (GlobalProfileCNN) models with Cerberus using any BigWig and BED file"
    )
    ...
    # Hyperparameters
    parser.add_argument(
        "--input-len", type=int, default=2048, help="Input sequence length"
    )
    parser.add_argument(
        "--output-len",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Number of channels in the global dense bottleneck",
    )
    parser.add_argument(
        "--background-ratio",
        type=float,
    ...
    parser.add_argument(
        ...
        "See cerberus.pseudocount.resolve_reads_equivalent_pseudocount.",
    )
    parser.add_argument(
        "--read-length",
        type=int,
