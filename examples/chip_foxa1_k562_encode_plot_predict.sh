#!/usr/bin/env bash
# Plot + prediction export for K562 ENCODE FOXA1 models trained by:
#   examples/chip_foxa1_k562_encode_bpnet_pomeranian.sh
#
...
#
# Usage:
#   bash examples/chip_foxa1_k562_encode_plot_predict.sh
#   bash examples/chip_foxa1_k562_encode_plot_predict.sh --multi
...
for RESIDUAL_ARCHITECTURE in "${RESIDUAL_ARCHITECTURES[@]}"; do
    for TRAINING_MODE in "${TRAINING_MODES[@]}"; do
        ...
        [[ -d "${MODEL_DIR}" ]] || {
            ...
            echo "Train first with examples/chip_foxa1_k562_encode_bpnet_pomeranian.sh" >&2
            exit 1
        }
