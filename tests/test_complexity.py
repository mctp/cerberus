...
def test_dust_comprehensive_normalization():
    ...
    test_cases = [
        ...
        (
            "NNNN",
            0.5,
        ),  # N repeat (L=4, k=3, M=2. "NNN" count=2. Score=2*1/2=1. Norm=1/2=0.5)
    ]
...
def test_cpg_comprehensive_normalization():
    ...
    for seq, expected_log_approx in test_cases:
        ...
        assert abs(raw - expected_log_approx) < 0.1, (
            f"Raw mismatch for {seq}: {raw} vs {expected_log_approx}"
        )
        ...
        assert abs(norm - expected_norm) < 1e-6, (
            f"Norm mismatch for {seq}. Raw={raw}, Norm={norm}"
        )
        assert 0.0 <= norm <= 1.0
