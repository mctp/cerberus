...
def test_nb_gradients_saturation():
    ...
    loss_fn = NegativeBinomialMultinomialLoss(
        total_count=r, count_weight=1.0, profile_weight=0.0
    )
...
def test_nb_convergence_to_poisson():
    ...
    # NB with huge r
    loss_nb = NegativeBinomialMultinomialLoss(
        total_count=1e9, count_weight=1.0, profile_weight=0.0
    )
    ...
    val_poisson = F.poisson_nll_loss(
        log_counts, targets.sum().view(1), log_input=True, full=False
    )
