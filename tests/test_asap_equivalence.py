...
class OriginalBasenjiCoreBlock(nn.Module):
    def __init__(
        ...
        for _ in range(self.nr_res_blocks):
            d_conv_block = OriginalConvBlock(
                filters_in,
                filters1,
                kernel_size=kernel1,
...
class OriginalConvNeXtDCNN(nn.Module):
    def __init__(self, **kwargs) -> None:
        ...
        self.init_conv = OriginalConvNeXtV2Block(
            channels_in=4,
        ...
        if self.use_map:
            self.unmap_predictor = OriginalUnmapPredictor(
                channels_in=config["filters0"]
            )

        self.core = OriginalBasenjiCoreBlock(
            nr_tracks=1,
            window=window,
    ...
    def forward(
        self, x: torch.Tensor, return_unmap=False
...
def disabled_test_convnext_dcnn_equivalence():
    ...
    original_model = OriginalConvNeXtDCNN(
        window_size=input_len, bin_size=output_bin_size, filters3=2048, use_map=False
    )
    original_model.eval()
    ...
    # Check shapes
    assert y_cerberus.shape == y_original_transposed.shape, (
        f"Shape mismatch: {y_cerberus.shape} vs {y_original_transposed.shape}"
    )
