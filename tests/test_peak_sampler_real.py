...
def test_peak_sampler_real_data(human_genome, mdapca2b_ar_dataset, tmp_path):
    ...
    with (
        gzip.open(original_intervals_path, "rt") as f_in,
        gzip.open(intervals_path, "wt") as f_out,
    ):
        count = 0
        for line in f_in:
            f_out.write(line)
    ...
    assert new_len == initial_len, (
        f"Sampler length changed after resampling! {initial_len} vs {new_len}"
    )
    ...
    assert overlap_count < new_len, (
        "Resampling produced identical intervals (negatives didn't change)"
    )
