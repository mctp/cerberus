    ...
    for interval in intervals:
        ...
        if interval.chrom == "chr1" and (interval.start, interval.end) in peak_tree:
            ...
            assert interval.chrom == "chr1", (
                f"Negative {interval} should be on chr1 to match GC"
            )

            # 5. Check Exclusion
            # Negative should not overlap any peak
            assert (interval.start, interval.end) not in peak_tree, (
                f"Negative {interval} overlaps peak"
            )
...
def test_nested_multi_sampler(peaks_bed, chrom_sizes, folds):
    ...
    # Outer: 15 items
    outer = MultiSampler(
        [inner, s3], chrom_sizes, folds=[], exclude_intervals={}, seed=2
    )
...
def test_create_sampler_errors(chrom_sizes, folds):
    ...
    with pytest.raises(
        ValueError, match="ComplexityMatchedSampler requires 'fasta_path'"
    ):
