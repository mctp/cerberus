...
Covers:
  1. compute_total_log_counts(implicit_log=True) — multi-channel MSE aggregation fix.
...
# ---------------------------------------------------------------------------
# 1. compute_total_log_counts — implicit_log=False (default, Poisson/NB space)
# ---------------------------------------------------------------------------
...
class TestComputeTotalLogCountsDefault:
    ...
    def test_single_channel_returns_log_counts_flat(self):
        ...
        result = compute_total_log_counts(out, implicit_log=False)
        assert result.shape == (1,)
        assert torch.isclose(result, torch.tensor([2.5]))

    def test_multi_channel_uses_logsumexp(self):
        """Multi-channel implicit_log=False: logsumexp gives log(sum(exp(log_counts)))."""
...
# ---------------------------------------------------------------------------
# 2. compute_total_log_counts — implicit_log=True (MSE / log1p space)
# ---------------------------------------------------------------------------

class TestComputeTotalLogCountsImplicitLog:

    def test_single_channel_unchanged(self):
        """Single channel: implicit_log has no effect (path returns flatten() directly)."""
        lc = torch.tensor([[math.log1p(42)]])
        out = ProfileCountOutput(logits=torch.zeros(1, 1, 4), log_counts=lc)
        assert torch.isclose(
            compute_total_log_counts(out, implicit_log=True),
            torch.tensor([math.log1p(42)]),
            atol=1e-6,
        )

    def test_multi_channel_gives_log1p_of_total(self):
        """Multi-channel implicit_log=True: result is log1p(c0+c1), not log(n_ch+c0+c1).
        ...
        result = compute_total_log_counts(out, implicit_log=True)
    ...
    def test_multi_channel_batched(self):
        """Batch of two intervals, multi-channel, implicit_log=True."""
    ...
    def test_implicit_log_ignored_for_profile_log_rates(self):
        """implicit_log is unused for ProfileLogRates — both flags give same result."""
        log_rates = torch.tensor([[[math.log(3), math.log(7)]]])
        out = ProfileLogRates(log_rates=log_rates)
        r_false = compute_total_log_counts(out, implicit_log=False)
        r_true  = compute_total_log_counts(out, implicit_log=True)
...
class TestObsLogCountFnByLossType:
    ...
    def test_mse_predicted_vs_observed_perfect_single_channel(self):
        ...
        out = ProfileCountOutput(logits=torch.zeros(1, 1, 4), log_counts=lc)
        pred_log = compute_total_log_counts(out, implicit_log=True)
    ...
    def test_poisson_predicted_vs_observed_perfect_single_channel(self):
        ...
        out = ProfileCountOutput(logits=torch.zeros(1, 1, 4), log_counts=lc)
        pred_log = compute_total_log_counts(out, implicit_log=False)
