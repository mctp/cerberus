...
def test_cache_reuse_across_splits(tmp_path):
    ...
    candidate_sampler = RandomSampler(
        chrom_sizes=chrom_sizes, padded_size=50, num_intervals=100, seed=42
    )
    ...
    target_sampler = ListSampler(
        intervals=target_intervals, chrom_sizes=chrom_sizes, folds=folds
    )

    candidate_sampler = RandomSampler(
        chrom_sizes=chrom_sizes, padded_size=50, num_intervals=100, folds=folds, seed=42
    )
    ...
    assert diff == 0, (
        f"Cache grew by {diff} entries, implying regeneration of candidates!"
    )
