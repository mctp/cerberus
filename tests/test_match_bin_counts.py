...
def test_match_bin_counts_1d_perfect_match():
    ...
    indices = match_bin_counts(
        target_metrics, candidate_metrics, bins=10, candidate_ratio=1.0, rng=rng
    )
...
def test_match_bin_counts_2d():
    ...
    candidate_metrics = np.vstack([[[0.1, 0.1]] * 20, [[0.9, 0.9]] * 20])
...
def test_match_bin_counts_ratio():
    ...
    indices = match_bin_counts(
        target_metrics, candidate_metrics, bins=10, candidate_ratio=2.0, rng=rng
    )
