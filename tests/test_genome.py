...
def test_genome_human_defaults(tmp_path):
    ...
    lines.extend(["chrX\t100\t0\t0\t0", "chrY\t100\t0\t0\t0", "chrM\t100\t0\t0\t0", "chrUn\t100\t0\t0\t0"])
    fai_path.write_text("\n".join(lines))
...
def test_genome_mouse_defaults(tmp_path):
    ...
    lines = [f"chr{c}\t100\t0\t0\t0" for c in range(1, 23)] # 1-22 (human range, so 20-22 are extra for mouse)
...
def test_encode_dna():
    ...
    # A -> 0 -> [1,0,0,0]
    assert torch.all(encoded[:, 0] == torch.tensor([1., 0., 0., 0.]))
    # C -> 1 -> [0,1,0,0]
    assert torch.all(encoded[:, 1] == torch.tensor([0., 1., 0., 0.]))
    # G -> 2 -> [0,0,1,0]
    assert torch.all(encoded[:, 2] == torch.tensor([0., 0., 1., 0.]))
    # T -> 3 -> [0,0,0,1]
    assert torch.all(encoded[:, 3] == torch.tensor([0., 0., 0., 1.]))
    # N -> -1 -> [0,0,0,0]
    assert torch.all(encoded[:, 4] == torch.tensor([0., 0., 0., 0.]))
...
def test_in_memory_sequence_extractor(tmp_path):
    ...
    assert torch.all(out[:, 0] == torch.tensor([1., 0., 0., 0.]))
    ...
    # 6: G -> col 0
    assert torch.all(out[:, 0] == torch.tensor([0., 0., 1., 0.]))
