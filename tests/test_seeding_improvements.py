...
def test_generate_sub_seeds_uniqueness():
    ...
    assert seeds1 != generate_sub_seeds(seed + 1, 5), (
        "Different master seeds should produce different sub-seeds"
    )
...
def test_random_sampler_resample():
    ...
    rs = RandomSampler(
        chrom_sizes=chrom_sizes, padded_size=10, num_intervals=5, seed=42
    )
    intervals1 = list(rs)
