...
class TestPeakSamplerNoBackground(unittest.TestCase):
    def test_zero_background_ratio(self):
        ...
        # Mock dependencies
        with (
            patch("cerberus.samplers.IntervalSampler") as MockIntervalSampler,
            patch("cerberus.samplers.RandomSampler") as MockRandomSampler,
            patch(
                "cerberus.samplers.ComplexityMatchedSampler"
            ) as MockComplexityMatchedSampler,
        ):
            # Setup Mock IntervalSampler (Positives)
            positives = MagicMock(spec=IntervalSampler)
            positives.__len__.return_value = 10
            positives.__iter__.return_value = iter(
                [Interval("chr1", i * 100, i * 100 + 50, "+") for i in range(10)]
            )
            positives.__getitem__.side_effect = lambda i: Interval(
                "chr1", i * 100, i * 100 + 50, "+"
            )
            # Make sure split_folds returns valid samplers (mocks)
            positives.split_folds.return_value = (
                MagicMock(spec=IntervalSampler),
                MagicMock(spec=IntervalSampler),
                MagicMock(spec=IntervalSampler),
            )
            ...
            print(
                "Verified: PeakSampler with background_ratio=0.0 does not create background samplers."
            )
