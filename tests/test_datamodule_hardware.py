...
def test_datamodule_hardware_settings():
    ...
    with (
        ...
        assert dm_explicit.pin_memory is False, (
            "pin_memory should remain False when explicitly set"
        )
        ...
        assert val_loader.persistent_workers is True, (
            "persistent_workers should be True in val_loader when enabled and num_workers > 0"
        )

        train_loader = dm.train_dataloader()
        assert train_loader.persistent_workers is True, (
            "persistent_workers should be True in train_loader when enabled and num_workers > 0"
        )
        ...
        assert val_loader.persistent_workers is False, (
            "persistent_workers should be False when explicitly disabled"
        )

        train_loader = dm_disabled.train_dataloader()
        assert train_loader.persistent_workers is False, (
            "persistent_workers should be False in train_loader when explicitly disabled"
        )
        ...
        assert val_loader.multiprocessing_context is None, (
            "Default multiprocessing_context should be None"
        )
        ...
        if val_loader.multiprocessing_context is not None:
            ...
            assert (
                "spawn" in ctx_repr.lower()
                or "Spawn" in type(val_loader.multiprocessing_context).__name__
