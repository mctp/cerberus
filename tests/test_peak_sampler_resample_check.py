...
def create_dummy_peaks(path):
    with open(path, "w") as f:
        ...
        for i in range(20):
            f.write(
                f"chr1\t{i * 200}\t{i * 200 + 100}\tpeak_{i}\t1000\t.\t50\t100\t-1\t-1\n"
            )
...
def test_peak_sampler_resampling_behavior(tmp_path):
    ...
    [
        iv for iv in train_intervals_1 if "peak" not in str(iv)
    ]  # We can't distinguish easily by name unless we track it.
    ...
    assert set(str(iv) for iv in train_intervals_1) != set(
        str(iv) for iv in train_intervals_2
    )
    ...
    assert set(str(iv) for iv in val_intervals_1) == set(
        str(iv) for iv in val_intervals_2
    )
    ...
    assert set(str(iv) for iv in val_intervals_1) != set(
        str(iv) for iv in val_intervals_3
    )
