...
def test_bpnet_loss_count_per_channel():
    ...
    outputs_perfect = ProfileCountOutput(
        logits=profile_logits, log_counts=pred_log_counts_perfect
    )
    ...
    outputs_bad = ProfileCountOutput(
        logits=profile_logits, log_counts=pred_log_counts_bad
    )
