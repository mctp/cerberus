...
class TestRandomnessConsistency(unittest.TestCase):
    def test_random_sampler_split_folds_idempotency(self):
        ...
        self.assertEqual(
            t1.seed,
            t2.seed,
            "RandomSampler split seeds should be idempotent given a fixed parent seed",
        )
    ...
    def test_multisampler_split_folds_seed_propagation(self):
        ...
        rs = RandomSampler(
            chrom_sizes, padded_size, num_intervals, folds=folds, seed=seed
        )
        ms = MultiSampler(
            [rs], chrom_sizes, folds=folds, exclude_intervals={}, seed=seed
        )
        ...
        self.assertIsNotNone(
            t1.seed, "MultiSampler split should have a seed derived from parent"
        )
        ...
        # Check idempotency
        self.assertEqual(
            t1.seed, t2.seed, "MultiSampler split seeds should be idempotent"
        )
        self.assertEqual(v1.seed, v2.seed)
        self.assertEqual(te1.seed, te2.seed)
    ...
    def test_multisampler_resample_updates_seed(self):
        ...
        self.assertNotEqual(
            seed1, seed2, "MultiSampler split seeds should change after resample(None)"
        )
    ...
    def test_complexity_matched_sampler_idempotency(self):
        ...
        candidate = RandomSampler(
            chrom_sizes, padded_size, num_intervals=100, folds=folds, seed=123
        )
        ...
        try:
            cerberus.samplers.compute_intervals_complexity = (
                lambda sampler, fasta, metrics, center_size=None: np.full(
                    (len(sampler), len(metrics)), 0.5
                )
            )

            gc = ComplexityMatchedSampler(
                target,
                candidate,
                "dummy.fa",
                chrom_sizes,
            ...
            self.assertEqual(
                te1.seed,
                te2.seed,
                "ComplexityMatchedSampler splits should have same seeds (idempotent)",
            )
            ...
            self.assertEqual(
                indices1,
                indices2,
                "ComplexityMatchedSampler splits should be idempotent",
            )
    ...
    def test_scaled_sampler_seed_derivation(self):
        ...
        self.assertNotEqual(
            ss.seed, rs.seed, "ScaledSampler should derive a sub-seed for its child"
        )
