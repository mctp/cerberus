...
def test_jitter_distribution():
    ...
    assert np.all(start_indices >= min_start), (
        f"Found start index {start_indices.min()} < {min_start}"
    )
    assert np.all(start_indices <= max_start), (
        f"Found start index {start_indices.max()} > {max_start}"
    )
