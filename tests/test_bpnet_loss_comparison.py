...
class TestBPNetLossComparison(unittest.TestCase):
    def test_loss_equivalence(self):
        ...
        true_counts = torch.floor(torch.abs(torch.randn(batch_size, channels, seq_len) * 10))
        ...
        self.assertTrue(torch.isclose(loss_cerberus_profile, loss_profile_ref, atol=1e-5))
        ...
        true_counts_1c = torch.floor(torch.abs(torch.randn(batch_size, 1, seq_len) * 10))
        ...
        loss_count_ref = F.mse_loss(log_counts_1c.squeeze(-1), target_counts_ref.squeeze(-1))
