...
def test_load_bed3(tmp_path):
    ...
    folds = create_genome_folds(
        chrom_sizes, fold_type="chrom_partition", fold_args={"k": 5}
    )
    sampler = IntervalSampler(
        f, chrom_sizes=chrom_sizes, padded_size=100, exclude_intervals={}, folds=folds
    )
...
def test_load_bed_padded_size(tmp_path):
    ...
    sampler = IntervalSampler(
        f, chrom_sizes=chrom_sizes, padded_size=50, exclude_intervals={}, folds=folds
    )
    intervals = list(sampler)
...
def test_load_narrowPeak_summit(tmp_path):
    ...
    sampler = IntervalSampler(
        f, chrom_sizes=chrom_sizes, padded_size=20, exclude_intervals={}, folds=folds
    )
    intervals = list(sampler)
    assert len(intervals) == 1
    # Center = 110
...
def test_load_bed_invalid_columns(tmp_path):
    ...
    with pytest.raises(ValueError, match="Invalid BED line"):
        IntervalSampler(
            f, chrom_sizes=chrom_sizes, padded_size=1, exclude_intervals={}, folds=folds
        )
...
def test_load_narrowPeak_with_chrom_sizes_and_padded_size(tmp_path):
    ...
    sampler = IntervalSampler(
        f, padded_size=20, chrom_sizes=chrom_sizes, exclude_intervals={}, folds=folds
    )
    intervals = list(sampler)
...
def test_sliding_window_basic():
    ...
    sampler = SlidingWindowSampler(
        chrom_sizes, padded_size=20, stride=20, exclude_intervals={}, folds=folds
    )
    intervals = list(sampler)
...
def test_sliding_window_overlap():
    ...
    sampler = SlidingWindowSampler(
        chrom_sizes, padded_size=20, stride=10, exclude_intervals={}, folds=folds
    )
    intervals = list(sampler)
...
def test_create_folds():
    ...
    folds = create_genome_folds(
        chrom_sizes, fold_type="chrom_partition", fold_args={"k": 2}
    )
    assert len(folds) == 2
