...
def test_random_sampler_split_dynamic_behavior(tmp_path):
    ...
    folds = create_genome_folds(
        chrom_sizes_multi, fold_type="chrom_partition", fold_args={"k": 5}
    )
    ...
    assert intervals_v1 != intervals_v2, (
        "RandomSampler split should allow resampling, but intervals remained static."
    )
