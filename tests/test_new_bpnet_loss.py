...
def test_bpnet_loss_alpha_beta():
    ...
    prof_loss_fn = MSEMultinomialLoss(count_weight=0.0, profile_weight=1.0, average_channels=True)
...
def test_poisson_multinomial_loss_average_channels():
    ...
    preds = ProfileCountOutput(logits=logits, log_counts=torch.zeros(batch_size, 1)) # Dummy counts
