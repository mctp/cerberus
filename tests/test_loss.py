...
def test_profile_mse_log1p_targets():
    ...
    logits_perfect_raw = torch.log(
        torch.tensor([[[10.0, 100.0]]])
    )  # Softmax matches raw probs
...
def test_bpnet_loss_profile_logic():
    ...
    loss_fn = MSEMultinomialLoss(
        count_weight=0.0, flatten_channels=True
    )  # Ignore count loss
    ...
    loss_good = loss_fn(
        ProfileCountOutput(logits=logits_good, log_counts=dummy_counts), targets
    )
    loss_bad = loss_fn(
        ProfileCountOutput(logits=logits_bad, log_counts=dummy_counts), targets
    )
...
def test_bpnet_loss_count_component_batch_gt_1():
    ...
    loss_perfect = loss_fn(
        ProfileCountOutput(logits=profile_logits, log_counts=pred_log_counts), targets
    )
    ...
    loss_bad = loss_fn(
        ProfileCountOutput(logits=profile_logits, log_counts=pred_log_counts_bad),
        targets,
    )
...
def test_bpnet_loss_with_log_transform():
    ...
    loss_std = loss_fn_std(
        ProfileCountOutput(logits=logits, log_counts=pred_log_counts), targets_raw
    )
    ...
    loss_wrong = loss_fn_wrong(
        ProfileCountOutput(logits=logits, log_counts=pred_log_counts), targets_logged
    )

    assert not torch.isclose(loss_std, loss_wrong), (
        "Loss should differ when targets are logged but not handled"
    )
    ...
    assert torch.isclose(loss_std, loss_fixed, atol=1e-5), (
        "Loss should match when log1p_targets is True"
    )
...
def test_poisson_multinomial_loss_log1p_targets():
    ...
    outputs = ProfileCountOutput(
        logits=torch.zeros(1, 1, 1), log_counts=torch.zeros(1, 1)
    )
...
def test_mse_multinomial_loss_count_pseudocount_default_equals_log1p():
    ...
    assert torch.allclose(
        perfect_log.grad, torch.zeros_like(perfect_log.grad), atol=1e-6
    )
...
def test_mse_multinomial_loss_count_pseudocount_custom():
    ...
    loss_fn_1(
        ProfileCountOutput(logits=torch.zeros(1, 1, 10), log_counts=pred_pseudo1),
        targets,
    ).backward()
    loss_fn_100(
        ProfileCountOutput(logits=torch.zeros(1, 1, 10), log_counts=pred_pseudo100),
        targets,
    ).backward()
    ...
    assert torch.allclose(
        pred_pseudo1.grad, torch.zeros_like(pred_pseudo1.grad), atol=1e-6
    )
    assert torch.allclose(
        pred_pseudo100.grad, torch.zeros_like(pred_pseudo100.grad), atol=1e-6
    )
    ...
    loss_fn_100(
        ProfileCountOutput(logits=torch.zeros(1, 1, 10), log_counts=pred_wrong), targets
    ).backward()
    assert pred_wrong.grad is not None
    assert not torch.allclose(
        pred_wrong.grad, torch.zeros_like(pred_wrong.grad), atol=1e-6
    )
