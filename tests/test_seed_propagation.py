...
def test_multisampler_seed_propagation():
    ...
    assert s1._indices != s2._indices, (
        "Sub-samplers should have different indices initially"
    )
    ...
    # s2 gets seed 1000
    assert s1._indices != s2._indices, (
        "Sub-samplers should have different indices after resample"
    )
...
def test_scaled_sampler_determinism():
    ...
    assert indices_1 != indices_2, (
        "Resample(None) should advance RNG and produce new indices"
    )
...
def test_scaled_sampler_split_folds_determinism():
    ...
    s.resample(
        None
    )  # Resets to sticky seed 43 if previous test passed, or whatever logic.
...
def test_multisampler_split_folds_seed_propagation_consistency():
    ...
    assert t1.seed is not None, (
        "MultiSampler split should have a seed derived from parent"
    )
