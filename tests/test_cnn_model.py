...
def test_vanilla_cnn_default():
    ...
    model = VanillaCNN(input_len=input_len, output_len=output_len, output_bin_size=output_bin_size, num_output_channels=num_output_channels)
...
def test_vanilla_cnn_large_input():
    ...
    model = VanillaCNN(input_len=input_len, output_len=output_len, output_bin_size=output_bin_size)
    x = torch.randn(batch_size, 4, input_len)
    output = model(x)
