...
def test_gopher_binning_options():
    ...
    # Bin size 1
    model = GlobalProfileCNN(
        input_len=input_len, output_len=output_len, output_bin_size=1
    )
    ...
    # Bin size 32
    model = GlobalProfileCNN(
        input_len=input_len, output_len=output_len, output_bin_size=32
    )
    output = model(torch.randn(batch_size, 4, input_len))
...
def test_gopher_bottleneck_config():
    ...
    model = GlobalProfileCNN(
        input_len=2048, output_len=1024, output_bin_size=4, bottleneck_channels=16
    )
    ...
    if explanation.graph_break_count > 0:
        ...
        if hasattr(explanation, "graph_breaks"):
            for break_info in explanation.graph_breaks:
                print(break_info)
        pytest.fail(
            f"Found {explanation.graph_break_count} graph breaks in GlobalProfileCNN"
        )
