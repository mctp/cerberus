...
def analyze_losses(output_len=1024, total_count=1000):
    ...
    log_counts_perfect = torch.log(torch.tensor([N], dtype=torch.float)).view(
        B, C
    )  # or (B,) depending on shape expectation
    ...
    ProfileCountOutput(logits=logits_perfect, log_counts=log_counts_perfect)

    # Case 2: Noisy prediction
    # Perturb logits and counts
    logits_noisy = logits_perfect + 0.1 * torch.randn_like(logits_perfect)
    log_counts_noisy = torch.log(torch.tensor([N * 1.1], dtype=torch.float)).view(
        B, C
    )  # 10% error in count
    ...
    loss_nb_multi = NegativeBinomialMultinomialLoss(
        count_weight=1.0, profile_weight=1.0, total_count=10.0
    )  # r=10
    ...
    print(
        f"MSEMultinomialLoss (Noisy): Count (MSE) = {c_mse:.4f}, Profile (MultiNLL) = {p_multi:.4f}"
    )

    # Poisson + Multinomial (CrossEntropy)
    c_poiss, p_ce = get_components(loss_poiss_multi, output_noisy, targets)
    print(
        f"PoissonMultinomialLoss (Noisy): Count (PoissNLL) = {c_poiss:.4f}, Profile (CE) = {p_ce:.4f}"
    )

    # NB + Multinomial
    c_nb, p_ce_nb = get_components(loss_nb_multi, output_noisy, targets)
    print(
        f"NegativeBinomialMultinomialLoss (Noisy): Count (NB) = {c_nb:.4f}, Profile (CE) = {p_ce_nb:.4f}"
    )
