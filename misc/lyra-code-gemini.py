...
class DropoutNd(nn.Module):
    ...
    def forward(self, X):
        ...
        mask = torch.empty(mask_shape, dtype=X.dtype, device=X.device).bernoulli_(
            1 - self.p
        )
        mask = mask / (1 - self.p)
        return X * mask
...
class S4DKernel(nn.Module):
    ...
    def register_parameter_custom(self, name, param, lr=None):
        ...
        getattr(self, name)._optim = {"weight_decay": 0.0, "lr": lr}
...
class Lyra(nn.Module):
        ...
        for hidden_dim, num_layers_pgc in pgc_configs:
            ...
            for _ in range(num_layers_pgc):
                self.pgc_layers.append(
                    PGC(model_dimension, expansion_factor=expansion, dropout=dropout)
                )
    ...
    def forward(self, x, return_embeddings=False):
        ...
        for layer, norm, dropout in zip(
            self.s4_layers, self.norms, self.dropouts, strict=False
        ):
            residual = x
            z = x
