...
from cerberus.samplers import IntervalSampler, RandomSampler, ComplexityMatchedSampler
...
def main():
    ...
    colors = {"Target": "blue", "Random Background": "gray", "Matched Candidates": "orange"}
    ...
    for i, t in enumerate(types):
        ...
        for j, (x_var, y_var) in enumerate(pairs):
            ...
            sns.scatterplot(
                data=subset, x=x_var, y=y_var, 
                ax=ax, color=color, alpha=0.1, s=5, edgecolor=None
            )
            ...
            try:
                sns.kdeplot(
                    data=subset, x=x_var, y=y_var, 
                    ax=ax, color=color, alpha=1.0, levels=5, linewidths=1.0
                )
    ...
    plt.suptitle(f"Complexity Matching Verification\n(Targets: {len(targets)}, Candidates Pool: {len(candidates)})", y=1.02)
    plt.tight_layout()
