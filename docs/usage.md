...
1. Loads `model.pt` (or best `.ckpt`) and the matching `hparams.yaml`.
2. Re-instantiates the trained single-task BPNet.
3. Wraps the model in `AttributionTarget` so upstream TPCAV sees a scalar-output `nn.Module`.
...
```bash
pip install -e ".[tpcav]"
# or combined interpretation extras
pip install -e ".[interpret]"
```
