...
*   `tools/train_bpnet.py`: Train a BPNet model.
    ...
    # Standalone ChromBPNet accessibility-sized BPNet
    python tools/train_bpnet.py --bigwig signal.bw --peaks regions.bed --output-dir models/my_bpnet \
        --filters 512 --n-layers 8 --conv-kernel-size 21 --dil-kernel-size 3 \
...
| `--precision` | `full` | `bf16` | `bf16` | `bf16` | `bf16` |
...
## Model Interpretation (TPCAV)

`tools/run_tpcav.py` is a thin Cerberus adapter around the upstream
[`tpcav`](https://github.com/seqcode/TPCAV) package.

The current MVP intentionally supports only:

- single-task `BPNet`
- sequence-only concepts (motif files)
- scalar targets wrapped through `AttributionTarget`

What the script does:

1. Loads the checkpoint via `ModelEnsemble(fold=args.fold)` (same layout as
   every other Cerberus inference tool).
2. Pulls the fold-specific model out of the ensemble.
3. Wraps it in `AttributionTarget` so upstream TPCAV sees a scalar-output `nn.Module`.
4. Resolves Cerberus-friendly probe-layer aliases such as `tower_out`.
5. Delegates concept construction, PCA fitting, CAV training, and HTML reporting to the upstream `tpcav` package.
...
Inspect available probe layers for a checkpoint:

```bash
python tools/run_tpcav.py \
    --checkpoint-dir models/my_bpnet/single-fold \
    --fold 0 \
    --list-layers
```

Default BPNet probe-layer aliases:

| Alias | Meaning |
|---|---|
| `initial_conv` | Initial sequence convolution (`iconv`) |
| `tower_block_0`, `tower_block_1`, ... | Individual dilated residual blocks |
| `tower_out` | Final shared tower representation before the heads |
| `profile_head` | Profile readout convolution |
| `count_head` | Count readout dense layer |

For BPNet, `tower_out` is the recommended default:

- `residual_pre-activation_conv` and `activated_residual_pre-activation_conv`:
  `final_tower_relu`
- `residual_post-activation_conv`:
  the last residual block output (`res_layers.<last>`)

Run TPCAV on motif concepts:

```bash
python tools/run_tpcav.py \
    --checkpoint-dir models/my_bpnet/single-fold \
    --fold 0 \
    --motif-file data/motifs/my_concepts.meme \
    --motif-file-fmt meme \
    --layer tower_out \
    --target-mode log_counts \
    --num-motif-insertions 4 8 16 \
    --num-samples-for-pca 10 \
    --num-samples-for-cav 1000 \
    --output-dir models/my_bpnet/single-fold/tpcav
```

Outputs:

- `run_config.json`: Cerberus-side run metadata
- `motif_auc_fscores.tsv`: saved when upstream TPCAV returns motif AUC/F-score summary
- upstream `cav_trainer.pt` artifacts (optional)
- upstream `report.html` (optional)

Notes:

1. `tools/run_tpcav.py` requires the upstream `tpcav` Python package in the active environment.
2. The script is deliberately thin: it does not reimplement concept generation or TCAV scoring.
3. Large motif databases can create very long-running jobs because upstream TPCAV trains one CAV per motif concept.
