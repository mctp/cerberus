...
A native ChromBPNet-style ATAC model composed of two BPNet branches:

* **Accessibility branch**: canonical large BPNet (`filters=512`, `n_dilated_layers=8` by default).
* **Bias branch**: smaller BPNet (`filters=128`, `n_dilated_layers=4` by default).
* **Combination rule**: profile logits are added directly; count logits are combined with `logaddexp`, matching `chrombpnet-pytorch`.
* **Shared bias mode**: `shared_bias=True` keeps the bias branch single-channel and broadcasts it across all accessibility channels. This is the multi-task scATAC use case: train the bias model once on one representative normalized sample, then reuse it across tasks from the same experimental procedure.
* **Return type**: plain `ProfileCountOutput`, so existing Cerberus losses, metrics, genome-wide prediction, and variant scoring work without any special-case logic.

Unlike [Dalmatian](#dalmatian), `ChromBPNet` does **not** use gradient routing or decomposed outputs. The reference workflow is recovered by loading a pre-trained bias BPNet into `bias_model` and freezing it during stage-2 training.

### Recommended Training Workflow

Stage 1: train the small BPNet bias branch on non-peak regions:

```bash
python tools/train_chrombpnet_bias.py \
    --bigwig signal.bw \
    --peaks peaks.bed \
    --output-dir models/chrombpnet_bias
```

Stage 2: train the full ChromBPNet with the frozen bias branch:
...
The stage-2 tool also exports `chrombpnet_wo_bias.pt` next to each full-model `model.pt`, matching the accessibility-only artifact commonly used for downstream interpretation and prediction. After training, it generates loss and Pearson curves from each Lightning `metrics.csv` and runs held-out test prediction evaluation by default. The evaluation writes compressed predicted-vs-observed log-counts and a companion metrics JSON under `predict/test/`.

For scATAC pseudobulk data generated by Cerberus, the example scripts consume the standard pipeline outputs directly: `bulk.bw` and `bulk_merge.narrowPeak.bed.gz` from `tools/scatac_pseudobulk.py`, with `target_scale=1.0` and the default count pseudocount of `1.0`.

### Multi-task ChromBPNet

**Implementation**: `cerberus.models.chrombpnet.MultitaskChromBPNet`

`MultitaskChromBPNet` is the shared-bias variant for multiple normalized ATAC/scATAC tasks. The accessibility branch has one profile/count head per task (`predict_total_count=False`), while the bias branch has one reusable channel. Use `MultitaskBPNetLoss` so profile and count losses are computed per task.

```bash
python tools/train_chrombpnet_multitask.py \
    --targets-json examples/scatac_kidney_multitask_targets.json \
    --peaks tests/data/scatac_kidney_pseudobulk/bulk_merge.narrowPeak.bed.gz \
    --pretrained-bias models/chrombpnet_bias/single-fold/fold_0/model.pt \
    --output-dir models/chrombpnet_multitask
```

The tool assumes normalization has already been applied upstream (for example CPM or quantile normalization). It does not normalize targets internally.
...
model_mt = MultitaskChromBPNet(
    input_len=2114,
    output_len=1000,
    input_channels=["A", "C", "G", "T"],
    output_channels=["PCT", "podocyte", "leukocyte"],
)
# logits:     (batch, 3, 1000)
# log_counts: (batch, 3)
```
