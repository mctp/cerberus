...
**Example Reference**: `notebooks/chip_ar_mdapca2b_predict_bpnet.py` shows how to load a trained BPNet model and run inference on test set peaks. `notebooks/chip_ar_mdapca2b_compare_models.py` extends this to compare both pretrained ensembles (BPNet and Pomeranian) on the full test interval fixture and produces predicted vs observed log-count scatter plots.
